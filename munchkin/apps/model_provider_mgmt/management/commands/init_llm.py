...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        obj, created = RerankProvider.objects.get_or_create(
            name="bce-reranker-base_v1", rerank_model=RerankModelChoices.BCE
        )
        if created:
            obj.rerank_config = {
            ...
            obj.save()
        obj, created = EmbedProvider.objects.get_or_create(
            name="bce-embedding-base_v1", embed_model=EmbedModelChoices.BCEEMBEDDING
        )
        if created:
            obj.embed_config = {
        ...
        obj, created = EmbedProvider.objects.get_or_create(
            name="text-embedding-ada-002", embed_model=EmbedModelChoices.OPENAI
        )
        if created:
            obj.embed_config = {
