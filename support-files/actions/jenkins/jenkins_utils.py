...
def get_jenkins_instance():
    crumb = CrumbRequester(server_settings.jenkins_username,
                           server_settings.jenkins_password,
                           baseurl=server_settings.jenkins_url,
                           ssl_verify=False, crumb_requester=True)
    jenkins = Jenkins(server_settings.jenkins_url,
...
def analyze_jenkins_build_console(job_name, build_number):
    ...
    result = ChatService().chat(
        'jenkins',
        f'扮演专业的运维开发工程师，你拥有丰富的运维领域经验，现在你正在使用Jenkins构建项目，出现了以下异常信息，'
        f'给出你的分析意见以及处置建议.:{build.get_console()}')
    return result
